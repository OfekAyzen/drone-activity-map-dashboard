...
- Backend: `pytest` (all of Phase 6, plus the stats tests added during the final audit) green - 42 tests; manually hit `POST /api/pipeline/run` against both sample files and confirm `pipeline_run` counters match the known valid/invalid split; hit `/docs` and exercise each `/api/drones` filter combination and `/api/drones/{id}` 404 case.
- Frontend: `ng test` (Phase 11 + Phase 16's store spec) green - 18 tests; manually run `ng serve`, trigger the pipeline from the UI, confirm markers appear/update, filters refetch and reset pagination, low-battery/lost-signal styling and path-history polyline render correctly on the sample data.
