...
- Backend: `pytest` (all of Phase 6 + stats tests from the final audit + Phase 18's `query_latest_drones` coverage) green - 52 tests; manually hit `POST /api/pipeline/run` against both sample files and confirm `pipeline_run` counters match the known valid/invalid split; hit `/docs` and exercise each `/api/drones` and `/api/drones/latest` filter combination and the `/api/drones/{id}` 404 case.
- Frontend: `ng test` (Phase 11 + Phase 16's store spec + Phase 18's listLatest/toggle coverage) green - 23 tests; manually run `ng serve`, trigger the pipeline from the UI, confirm markers appear/update, filters refetch and reset pagination, low-battery/lost-signal styling and path-history polyline render correctly on the sample data, and toggling "Show latest position per drone only" shows every active drone even when one is transmitting far more frequently than the others.
